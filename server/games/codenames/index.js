...
class CodenamesInstance {
  constructor(room, emitEvent) {
    ...
    this.settings = Object.assign({
      ...
      timerPerTurn: 120                // seconds (default 120s / 2 mins)
  ...
  setupNewGame() {
    ...
    const startingRuleText = this.settings.startingTeamMode === 'equal' ? 'Equal 8 vs 8 cards' : `${this.startingTeam.toUpperCase()} team starts (${this.startingTeam === 'red' ? this.redRemaining : this.blueRemaining} cards)`;
    this.log = [{
      type: 'system',
      text: `🎮 Game Started! ${startingRuleText}. 25 cards dealt. Active Spymaster: Submit your clue!`
    }];

    if (this.settings.timerPerTurn > 0) {
      this.resetTurnTimer();
    }
  }
  ...
  resetTurnTimer() {
    this.stopTurnTimer();
    const duration = (this.settings.timerPerTurn !== undefined && this.settings.timerPerTurn !== null) ? this.settings.timerPerTurn : 120;
